...
export default function RecruiterTerminal({
  ...
  const keyMetrics = keyMetricsOverride.length > 0 ? keyMetricsOverride : content.recruiterPayloadData.keyMetrics;
  const projectRoi = projectRoiOverride.length > 0 ? projectRoiOverride : content.recruiterPayloadData.projectRoi;

  return (
    <section
      id="terminal"
      style={sectionStyle}
      className="relative w-full px-6 flex justify-center z-20 scroll-mt-[100px]"
    >
      <div className="w-full max-w-4xl bg-[#000000] border-neo-black shadow-[8px_8px_0px_0px_#FACC15] flex flex-col p-6 md:p-8 rounded-2xl">
        ...
        <div className="flex items-center gap-4 border-b-2 border-[#10B981] pb-4 mb-6">
          <div className="h-3 w-3 bg-[#FF6B9E] rounded-full animate-pulse" />
          <h2 className="font-mono text-sm md:text-base font-black text-[#10B981] uppercase flex items-center gap-2">
            ...
            <span className="inline-block w-3 h-5 bg-[#10B981] animate-pulse" />
          </h2>
        </div>

        <div className="flex flex-col gap-2 w-full font-mono text-sm md:text-base font-bold text-white uppercase">
          <a href="/cv.pdf" download className="w-full flex items-center p-3 md:hover:bg-[#10B981]/20 md:hover:text-[#10B981] transition-colors cursor-pointer outline-none focus-visible:ring-4 focus-visible:ring-[#FF6B9E]">
            &rarr; DOWNLOAD CV
          </a>
          ...
          <div className="w-full flex flex-col">
            ...
            <AnimatePresence>
              {activeDrawer === 'metrics' && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-white/5 border-l-2 border-[#10B981] ml-4 mt-2">
          ...
          <a href="#contact" className="w-full flex items-center p-3 md:hover:bg-[#10B981]/20 md:hover:text-[#10B981] transition-colors cursor-pointer outline-none focus-visible:ring-4 focus-visible:ring-[#FF6B9E]">
            &rarr; CONTACT DIRECT
          </a>
        </div>
      </div>
