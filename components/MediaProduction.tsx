      ...
      <div className="mx-auto max-w-7xl relative z-10">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-12 md:gap-8 mt-24">
          {cards.map((card, idx) => {
            ...
            return (
              <div key={idx} className={`w-full max-w-sm rounded-2xl border-neo-black bg-white p-6 shadow-neo-black cursor-pointer active-neo-press z-20 md:hover:scale-105 transition-transform duration-200 ${card.rotation}`}>
                <div className="aspect-[4/5] w-full rounded-xl border-4 border-neoblack bg-neocream flex flex-col items-center justify-center relative overflow-hidden mb-6">
                  {card.imageUrl ? (
                    ...
                    <>
                      ...
                      <span className="font-mono text-xs font-bold text-neoblack mt-4 uppercase opacity-50">[ADD PHOTO URL IN NOTION]</span>
                    </>
                  )}
                ...
                <h3 className="font-display text-2xl font-black uppercase tracking-tighter leading-none text-neoblack mb-2">{card.title}</h3>
                <p className="font-body text-base font-medium text-neoblack/80 mb-6">{card.description}</p>
