      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-16">
          <h2
            ...
            className="font-display text-[clamp(3rem,6vw,5rem)] font-black uppercase tracking-tighter text-neoblack"
          >
            {heading} <br />
          ...
          <div className="group relative z-20 border-neo-black bg-white shadow-neo-black rounded-3xl overflow-hidden cursor-pointer active-neo-press">
            <div className="flex w-full flex-col lg:flex-row">
              <div className="flex flex-col justify-between p-8 lg:w-1/2 lg:p-12 relative z-10 bg-white">
                ...
                <div className="mt-12 flex flex-wrap gap-3">
                  {project1Tech.map((tech, i) => (
                    <div key={i} className="rounded-full border-[3px] border-neoblack bg-white px-4 py-2 font-mono text-sm font-bold uppercase text-neoblack shadow-[2px_2px_0px_0px_#000]">
                      {tech}
                    </div>
                  ))}
                </div>
              ...
              <div className="relative flex min-h-[300px] lg:min-h-full items-center justify-center border-t-4 border-neoblack lg:border-t-0 lg:border-l-4 lg:w-1/2 bg-neogreen p-8 overflow-hidden z-0">
                <div className="absolute top-8 right-8 h-12 w-12 rounded-full border-neo-black bg-neoyellow shadow-[4px_4px_0px_0px_#000]" />
                <div className="absolute bottom-8 left-8 h-16 w-16 border-neo-black bg-neoblue shadow-[4px_4px_0px_0px_#000] rotate-12" />
                {project1ImageUrl ? (
                  <div className="w-[80%] rounded-2xl border-neo-black bg-white shadow-neo-black z-10 overflow-hidden hover:-translate-y-2 transition-transform duration-200 aspect-video relative">
                    <Image src={project1ImageUrl} alt={project1Name} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" priority />
                  </div>
                ) : (
                  <div className="w-[80%] rounded-2xl border-neo-black bg-white p-6 shadow-neo-black z-10 hover:-translate-y-2 transition-transform duration-200">
                    <div className="mb-4 flex gap-2 border-b-4 border-neoblack pb-4">
                      <div className="h-4 w-4 rounded-full border-2 border-neoblack bg-neopink" />
                      <div className="h-4 w-4 rounded-full border-2 border-neoblack bg-neoyellow" />
                      <div className="h-4 w-4 rounded-full border-2 border-neoblack bg-neoblue" />
                    </div>
              ...
              <div className="flex flex-col justify-between p-8 lg:w-1/2 lg:p-12 relative z-10 bg-white border-b-4 lg:border-b-0 lg:border-l-4 border-neoblack">
                <div>
                  <div className="mb-6 inline-block rounded-full border-neo-black bg-neoyellow px-4 py-1 shadow-[2px_2px_0px_0px_#000]">
                    <span className="font-mono text-xs font-bold tracking-widest uppercase text-neoblack">{p2Tag}</span>
                  </div>
                  <h3 className="font-display text-4xl font-black uppercase md:text-6xl text-neoblack opacity-50">{p2Name}</h3>
                  <p className="mt-6 font-body text-xl font-medium text-neoblack/70">{p2Description}</p>
                </div>
                <div className="mt-12 flex flex-wrap gap-3">
                  <div className="rounded-full border-[3px] border-neoblack bg-neocream px-4 py-2 font-mono text-sm font-bold uppercase text-neoblack opacity-70">CLASSIFIED TECH</div>
                </div>
              </div>
