...
export default function Contact({ heading, cta }: ContactProps) {
  ...
  const getInputClass = (id: string) => {
    ...
    return `w-full border-neo-black p-4 font-body text-lg text-neoblack outline-none focus-visible:ring-4 focus-visible:ring-[#FF6B9E] focus-visible:ring-offset-2 focus-visible:ring-offset-[#FFF9F0] transition-colors rounded-none placeholder:text-neoblack/40 z-20 relative ${
      isFocused ? "bg-neopink shadow-neo-black" : "bg-white"
    }`;
  };

  return (
    <section id="contact" className="relative w-full bg-neocream py-32 px-6 scroll-mt-[100px]">
        ...
        <div className="rounded-3xl border-neo-black bg-neoblue p-8 shadow-neo-black md:p-12 relative z-10 overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 relative z-20">
            <div className="flex flex-col">
              <label htmlFor="name" className="mb-2 font-mono text-sm font-bold uppercase text-neoblack">
                IDENTIFIER // NAME
              </label>
              <input 
                type="text" 
            ...
            <div className="flex flex-col">
              <label htmlFor="email" className="mb-2 font-mono text-sm font-bold uppercase text-neoblack">
                COORDINATES // EMAIL
              </label>
              <input 
                type="email" 
            ...
            <div className="flex flex-col">
              <label htmlFor="message" className="mb-2 font-mono text-sm font-bold uppercase text-neoblack">
                PAYLOAD // MESSAGE
              </label>
              <textarea 
                id="message" 
          ...
          <div className="absolute -bottom-10 -right-10 h-40 w-40 border-neo-black bg-neogreen z-0 rotate-12"></div>
        </div>
      </div>
    </section>
  );
