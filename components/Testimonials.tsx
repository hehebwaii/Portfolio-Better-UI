...
interface TestimonialOverride { id: string; name: string; role: string; text: string; }
...
export default function Testimonials({
  ...
  const testimonials = testimonialOverrides.length > 0 ? testimonialOverrides : content.testimonials;
  ...
  return (
    <section
      id="testimonials"
      style={sectionStyle}
      className="relative w-full bg-neocream py-32 px-6 flex flex-col items-center scroll-mt-[100px]"
    >
      ...
      <div className="relative z-20 cursor-pointer active-neo-press hover:scale-105 transition-transform" onClick={() => setIsOpen(!isOpen)}>
      ...
      <div className="w-full max-w-5xl mt-24">
        <AnimatePresence>
          {isOpen && (
            <motion.div initial={{ opacity: 0, y: -40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -40 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {testimonials.map((testimonial, i) => (
                <motion.div key={testimonial.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} transition={{ delay: i * 0.1 }} className="rounded-2xl border-neo-black bg-white p-8 shadow-neo-black z-10 hover:-translate-y-2 transition-transform duration-200">
                  <p className="font-body text-base font-bold text-neoblack leading-relaxed mb-8">&ldquo;{testimonial.text}&rdquo;</p>
                  <div className="border-t-4 border-neoblack pt-4 flex flex-col">
                    <span className="font-display text-xl font-black uppercase text-neoblack opacity-50">{testimonial.name}</span>
                    <span className="font-mono text-xs font-bold bg-neoyellow border-2 border-neoblack px-2 py-1 mt-2 self-start uppercase">{testimonial.role}</span>
                  </div>
                </motion.div>
              ))}
            </motion.div>
